...
if (window.location.pathname.includes('admin.html') || document.getElementById('adminPanel')) {
  ...
  const API_URL = window.API_URL = "https://script.google.com/macros/s/AKfycbxLuEaJBoQV7VHBoOnoWniFa3eVhuZhFix2anUYATw2b0CrQ4ofPAjWGBF3HltPTbbO/exec";
  ...
  if (productosDiv) { // Solo inicializar si estamos en index.html
    // Mostrar skeleton inmediatamente (feedback visual mientras carga GAS)
    renderSkeletonProductos(8);
    mostrarProductos();

    async function inicializarApp(intentoNum = 1) {
      try {
        console.log(`🚀 Inicializando aplicación (intento ${intentoNum})...`);
        ...
        if (productos.length === 0) {
          throw new Error("Sin productos del servidor");
        }
        ...
        const errEl = document.getElementById("_initErrorBanner");
        if (errEl) errEl.remove();

        console.log("✅ Aplicación lista");
      } catch (error) {
        console.error(`❌ Error en inicialización (intento ${intentoNum}):`, error);

        if (intentoNum < 3) {
          // Reintento automático — resuelve cold start de GAS (primera petición tarda más)
          const delay = intentoNum * 4000;
          console.warn(`⏳ Reintentando en ${delay / 1000}s...`);
          cache.productos = null;
          cache.categorias = null;
          await new Promise(r => setTimeout(r, delay));
          return inicializarApp(intentoNum + 1);
        }

        // Tras 3 intentos fallidos: mostrar UI de error con botón de recarga
        mostrarCategorias();
        const grid = document.getElementById("categoriasGrid");
        if (grid) {
          grid.innerHTML = `
          <div id="_initErrorBanner" style="
            grid-column:1/-1; padding:40px 24px; text-align:center;
            background:#fff3f3; border-radius:16px; border:1.5px solid #ffcdd2;
          ">
            <div style="font-size:48px;margin-bottom:12px">📡</div>
            <p style="font-weight:600;font-size:16px;color:#c62828;margin-bottom:6px">
              No se pudo conectar con el servidor
            </p>
            <p style="font-size:14px;color:#666;margin-bottom:20px">
              El servidor puede estar iniciando (cold start). Por favor intenta de nuevo.
            </p>
